...
#   thm   : Binarization method (B: simple binary, G: gaussian, O: Otsu). Default: Gaussian (G))
#   inv   : Invert binary mask (y/n)
# -----------------------------------------------------------------------------------------------------------------------------
...
ap.add_argument("-fmin", "--filtmin", required=False, default='10', help="Mininum area filter (in units**2). Default value:10 pixels")
...
ap.add_argument("-thm", "--thresholdmethod", required=False, default='G', help="Binarization method (B: simple binary, G: gaussian, O: Otsu). Default: Gaussian (G))")
ap.add_argument("-inv", "--invert", required=False, default='n', help="Invert binary mask (y/n). Default: No")
...
splitb=args["split"]
thrmethod=args["thresholdmethod"]
invert=args["invert"]
...
f2.write('ID,L,W,Area,perimeter,xc,yc\n')
...
if(thrmethod == 'B'):
    if(invert == 'y'):
        ret,patt = cv2.threshold(gray,tresh_8b_min,tresh_8b_max,cv2.THRESH_BINARY_INV)
    else:
        ret,patt = cv2.threshold(gray,tresh_8b_min,tresh_8b_max,cv2.THRESH_BINARY)
elif(thrmethod == 'G'):
    if(invert =='y'):
        patt = cv2.adaptiveThreshold(gray,tresh_8b_max,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY_INV,11,2)
    else:
        patt = cv2.adaptiveThreshold(gray,tresh_8b_max,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY,11,2)        
    cv2.imwrite("out_"+dirstr+"/"+dirstr+"_binary.jpg", patt)
elif(thrmethod == 'O'):
    blur = cv2.GaussianBlur(gray,(5,5),0)
    if(invert =='y'):
        ret,patt = cv2.threshold(blur,0,255,cv2.THRESH_BINARY_INV+cv2.THRESH_OTSU)
    else:
        ret,patt = cv2.threshold(blur,0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
...
for j in range(0,len(contours),1):
    ...
    if (area > filtareamin) & (area < filtareamax):
        ...
        if(splitb == "y"):
            cv2.imwrite("out_"+dirstr+"/split/filtobj_"+str(nfilt)+".jpg", contpic[yminb:ymaxb,xminb:xmaxb,:])
...
cv2.imwrite("out_"+dirstr+"/"+dirstr+"_allcont.jpg", contpic)
f2.close()
...
plt.savefig("out_"+dirstr+"/"+dirstr+"_histplot.jpg",dpi=200)
...
plt.savefig("out_"+dirstr+"/"+dirstr+"_boxsplot.jpg",dpi=200)
